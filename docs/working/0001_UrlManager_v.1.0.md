...
class TestContext:
    ...
    # Optional HTTP body data per method, keyed by method name (uppercase).
    # Required for POST/PUT/PATCH to avoid Django returning 500 on empty body.
    # E.g.: {"POST": {"name": "test", "slug": "test-entry"}, "PUT": {...}}
    # Methods not present in this dict will be called with no body (fine for GET/DELETE).
...
class BaseDataProvider(ABC):
    ...
    def get_test_context(
        ...
        4. method_data: dict of HTTP body data keyed by method name (e.g. {"POST": {...}})
           -- required for POST/PUT/PATCH methods to avoid Django returning 500 on empty body.
           The DataProvider is informed about which methods the URL handles (via
           url.supported_methods) and should supply minimal valid data for each write method.
...
class ContestDataProvider(BaseDataProvider):
    ...
    def _get_method_data(self, url: DiscoveredUrl) -> dict[str, dict]:
        """
        Return minimal valid HTTP body data for each write method the URL supports.
        Called for every URL so that POST/PUT/PATCH tests don't receive an empty body
        and return 500. Read-only methods (GET, DELETE, HEAD) need no data.

        Example: for an entry creation form URL
            {"POST": {"name": "Test Entry", "style": 1, ...}}
        """
        data: dict[str, dict] = {}
        for method in url.supported_methods:
            if method in (HttpMethod.POST, HttpMethod.PUT, HttpMethod.PATCH):
                data[method.value] = self._minimal_form_data(url, method)
        return data

    def _minimal_form_data(self, url: DiscoveredUrl, method: HttpMethod) -> dict:
        """Override per URL to provide valid form data. Returns empty dict by default."""
        return {}
...
class AccessTester:
    ...
    def test_access(
        ...
        return self._execute(ctx.resolved_url, method, user, rules,
                             ...
                             subcategory=subcategory, ctx=ctx)
    ...
    def _make_request(
        ...
        data: optional HTTP body for POST/PUT/PATCH (from TestContext.method_data).
        """
    ...
    def _execute(
        ...
        request_data = ctx.method_data.get(method.value) if ctx else None
...
UrlManager extracts **actually handled** methods from views using the same `hasattr` approach as the `list_urls` management command:

```python
def _extract_methods(self, view) -> list[HttpMethod]:
    """
    Use hasattr() to check which HTTP method handlers are present on the view.
    For CBVs: unwrap view_class first (callback.view_class if present).
    For FBVs: use the callback directly.

    Mirrors the approach in list_urls management command:
...
def _get_view(self, callback) -> type:
    """Unwrap CBV to get view class, or return FBV callback directly."""
...
| `by_capability/{cap}` | Anonymous user | tested directly (no user) | Matches `no_access_rules` |
...
| `by_group/{group}` | Anonymous user | tested directly (no user) | Matches `no_access_rules` |
| `by_group/{group}` | Basic user (not in group) | `unauthorized_users=[basic_user]` | Matches `no_access_rules` |

**Rule:** Anonymous denial is tested against **all non-anonymous groups** using that group's own `no_access_rules`. This is separate from the DataProvider's `unauthorized_users` list (which provides logged-in users without access).
...
| **Stats Recording** | |
| `test_post_data_passed_to_client` | method_data["POST"] is passed as data= to client.post() |
| `test_no_data_for_get` | GET requests pass no data= even if method_data present |
...
**Decision:** Empty POST bodies cause Django to return 500 (unexpected input). `TestContext` includes `method_data: dict[str, dict]` supplying minimal valid form data per HTTP method. `DataProvider._get_method_data()` builds this dict from `url.supported_methods`. `AccessTester._make_request()` passes it as `data=` to the Django test client. `_minimal_form_data()` defaults to empty dict and is overridden per URL in the project-specific DataProvider.

### 15.2 Method Extraction for CBVs

**Decision:** Use `hasattr(view, method_name)` for each of `get/post/put/patch/delete` — the same approach as the existing `list_urls` management command. CBVs are unwrapped via `callback.view_class` first. This is simple, proven, and consistent with the rest of the codebase.

### 15.3 Anonymous Denial Tests for Non-Anonymous Groups

**Decision:** Anonymous users are tested for denial against **all non-anonymous groups** (`logged_in`, `by_capability`, `by_group`). Each group's own `no_access_rules` is used for the assertion — capability URLs may redirect to `contest_detail` rather than `account_login`, and this will be respected. See Section 10.2 and Section 10.4.
